...
//		ringDateTime	-- 日期/时间类
//
//感谢YZFree为RingSDK重整了整个工程，并编写了DEBUG版本的
//内存操作函数New和Del
//
**********************************************************************/
#ifndef _INC_RINGSDK
...
//类库初始化
void InitRingLib(HWND hWnd,HINSTANCE hInst = NULL);
...
//执行文件（包括可执行文件和文档）
HINSTANCE ringExec(LPCTSTR lpFile,LPCTSTR lpDir=NULL,LPCTSTR lpParam=NULL,
...
public:
	//构造，复制构造函数
	ringStr():m_data(NULL),m_size(0),m_wData(NULL),m_len(0){memset(x,0,4);}
	ringStr(LPCTSTR str,DWORD size=0);
	//ringStr(LPSTR format,...);
	ringStr(int size);		//size:初始长度，<0且在RSTR_WINPATH~RSTR_PCNAME范围内为特殊字串
	ringStr(const ringStr& src);
	~ringStr();
	...
	const ringStr& operator =(LPCSTR str);
	const ringStr& operator =(LPCWSTR str);
	const ringStr& operator =(TCHAR ch);
	const ringStr& operator =(const unsigned char* psz);
	const ringStr& operator =(const ringStr& str);
	const ringStr& operator =(const int number);
	const ringStr& operator +=(const int number);
	const ringStr& operator +=(const ringStr& str);
	const ringStr& operator +=(LPCTSTR lpstr);
	const ringStr& operator +=(TCHAR ch);

	ringStr operator +(const ringStr& rhs) const;
	ringStr operator +(TCHAR ch) const;
	friend ringStr operator +(LPCTSTR lhs, const ringStr& rhs);
	friend ringStr operator +(TCHAR ch, const ringStr& rhs);

#ifdef _UNICODE
	const ringStr& operator=(char ch);
	const ringStr& operator +=(char ch);
	friend ringStr operator +(char ch, const ringStr& rhs);
#endif	
	// Comparisons
	BOOL operator ==(const ringStr& str) const;
	...
	int cmp(const ringStr& str) const;
	int cmpi(const ringStr& str) const; //ignorecase
	
	int GetLength();
	int len() const{return m_len;}
	LPCTSTR string() const {return (m_data)?m_data:"";}
	...
	ringStr replace(LPCTSTR str1,LPCTSTR str2,BOOL bAll = TRUE) const;
	BOOL replace(ringStr& temp,LPCTSTR str1,LPCTSTR str2,BOOL bAll = TRUE) const;
	void replace(TCHAR c1,TCHAR c2,BOOL bAll = TRUE);
	ringStr SubString(int start,int count = 0) const;
	//取匹配的中间字串，例：{123456}
	//SubString("{","}"),返回123456,nIndex返回最后}的位置
	ringStr SubString(LPCTSTR szStart,LPCTSTR szEnd,int nStart=0,int* nIndex=NULL) const;
	...
	ringStr Insert(const ringStr& str, int index);
	ringStr Delete(int index, int count);
...
extern ringStr operator +(LPTSTR,const ringStr& rhs);
...
public:
   ...
   //某个键下取值开始（不关闭该键，调用其他函数会自动关闭）
	BOOL Get(HKEY hKey,LPCTSTR szSubKey,LPCTSTR szKey,LPVOID lpData,DWORD dwDataSize);
   DWORD GetInt(HKEY hKey,LPCTSTR szSubKey,LPCTSTR szKey,DWORD dwDef = 0);	//dwDef:失败时返回的默认值
   //在已打开的键下取值，以下两个函数必须在调用上面两个函数之一后调用
   BOOL Get(LPCTSTR szKey,LPVOID lpData,DWORD dwDataSize);
...
public:
	// 预关联操作
	ringFile();
	ringFile(LPCTSTR szFilename);
	ringFile& operator=(LPCTSTR szFilename){SetFile(szFilename);return *this;}
	...
	// 实关联操作
	ringFile(HANDLE hFile){Init();SetFile(hFile);}
	...
	{
		if(Write((LPVOID)lpBuf,strlen(lpBuf)))
			return Write("\r\n",2);
		else
			return FALSE;
	}
	...
	static unsigned __int64 GetDiskFreeSize(LPSTR lpRoot);
...
class ringMappingFile:public ringFile
{
public:
	ringMappingFile():m_hMapping(NULL){}
	ringMappingFile(HANDLE hFile):ringFile(hFile),m_hMapping(NULL){}
	virtual ~ringMappingFile();
...
public:
	...
	virtual BOOL	Create(LPCSTR lpFilename);
	virtual BOOL	Open(LPCSTR lpFilename);
...
public:
   ...
   BOOL SetDate(WORD month = 0,WORD day = 0,WORD year = 0);
   BOOL SetTime(WORD hour = 61,WORD minute = 61,WORD second = 61);

	//LPCTSTR FormatString(LPCTSTR szFormat);

	ringDateTime& operator =(const ringDateTime& rdt);
	//采用以下方式赋值自动置m_bReCount为FALSE，即不与系统时间绑定。	
	ringDateTime& operator =(const SYSTEMTIME* lptime);
	ringDateTime& operator =(const FILETIME* lptime);
		
	SYSTEMTIME* IncMonth(int nStep,SYSTEMTIME* lptime = NULL);
	
protected:
	LPCTSTR formatStr(LPSTR s,int nval,int length);
...
#define RINGFONT_BOLD				1	//黑体
#define RINGFONT_ITALIC				2	//斜体
#define RINGFONT_UNDERLINE			4	//下划线
#define RINGFONT_STRIKEOUT			8	//删除线
#define RINGFONT_PIXEL				16	//字体大小为像素

class ringFont
{
public:
	ringFont();
	ringFont(HFONT hfont){SetFont(hfont);}
	ringFont(LPCTSTR szFontname,int size,int nStyle=0,int angle=0);
	~ringFont();
	//nStyle:=LOGFONT结构lfWeight值+RINGFONT_xxx一个或多个的或（|）值
	...
	void SetWindowFont(HWND hWnd,LPCTSTR szFontname=NULL,int size=0,int nStyle=0);
...
//
*****************************************************/
#define RINGMSG_NONE				0		//不处理
#define RINGMSG_LOGFILE			1		//记录到文件
#define RINGMSG_SENDMESSAGE	2		//发送消息
#define RINGMSG_ALERT			4		//弹出消息框显示
#define RINGMSG_DEBUGOUT		8		//显示到Debugger窗口
#define RINGMSG_RECTIME			0x00010000		//记录该显示该信息的时间
#define RINGMSG_NORETURN		0xF0000000		//仅用于EDIT控件显示，无回车
...
public:
	...
	LPCSTR GetLastMsg(){return m_str;}
...
private:
	...
	friend class RingMsgClass;
};

/*****************************************************
//
//信息显示类
//		对RingMsg的再次封装，以RingMsg的对象指针为成员函数，
//		供需要信息显示功能的类自本类继承，以灵活方便配置。
//		提供该类主要原因是有些类可能需要信息显示，也可能不
//		需要，如果继承自RingMsgs则可能包含无用代码和数据，
...
{
public:
	RingMsgClass():m_msg(NULL){}
	virtual ~RingMsgClass(){delete m_msg;}
...
{
public:
	RingDataGuard(){InitializeCriticalSection(&m_crs);}
	~RingDataGuard(){DeleteCriticalSection(&m_crs);}
...
//RINGVCLASS类作用为转换各类的成员数据和成员函数的指针类型，
//该类本可仅声明，无任何定义和实现，但在BC运行正常，
//VC则表现为sizeof(RINGVCLASS)异常，因此必须有实现(加{}就行)。
class RINGVCLASS
{
//public:
//	RINGVCLASS(){}
...
typedef RingThreadInfo<RINGVCLASS> RingThInfo;
typedef RingThreadInfo<RINGVCLASS>::THREADFUNC1 THREADFUNC;
...
public:
	...
	{		
		...
		RingThreadInfo<tname>* lpv;
		if(m_info)
		{
			if((LONG)m_info->m_obj == (LONG)obj)
				lpv = (RingThreadInfo<tname>*)m_info;
			else
			{
				delete m_info;
				lpv = new RingThreadInfo<tname>;
			}
		}
		else
			lpv = new RingThreadInfo<tname>;
		...
		lpv->m_thread = this;
		lpv->m_param = param;
		lpv->m_lpthreadFunc = (RingThreadInfo<tname>::THREADFUNC1)func;
		
		m_info = (RingThreadInfo<RINGVCLASS>*)lpv;
...
private:
	static DWORD WINAPI ThreadProc(RingThInfo* obj);
	static DWORD WINAPI QuitProc(RingThread* obj);
	//通知线程结束（由线程处理函数退出时调用）
...
public:
	...
	LPCTSTR GetLogFileName(){return (m_rf?m_rf->Filename():"");}
...
private:
	...
	BOOL m_bNewLine;
	LPSTR m_lpszErr;
	char m_szFilePrefix[MAX_PATH],m_szDate[32];
	RingDataGuard m_ctl;	
	RingThread* m_thread;
	ringDateTime m_rdt;
	ringFile* m_rf;
...
#ifdef _DEBUG
#pragma comment(lib, "ringlibd.lib")
#else
#pragma comment(lib, "ringlib.lib")
#endif
