...
#include <windows.h>//ringlib\ringlib.h>
#include "JpegFile.h"
...
BYTE * JpegFileToRGB(LPSTR fileName,UINT *width,UINT *height)

{
   ...
   {
		//wsprintf(buf, "错误：不能打开文件\"%s\"", fileName);
	...
	if (dataBuf==NULL)
   {
      ...
      //MessageBox(GetFocus(),"内存不足","错误",MB_OK|MB_ICONWARNING);
		jpeg_destroy_decompress(&cinfo);
		fclose(infile);
		return NULL;
	}
...
BOOL GetJPGSize(LPSTR fileName,UINT *width,UINT *height)
...
BOOL RGBToJpegFile(LPSTR fileName,BYTE *dataBuf,UINT widthPix,UINT height,BOOL bRGB,int quality)
{
	if (dataBuf==NULL)
		return FALSE;
	if (widthPix==0)
...
BOOL BGRFromRGB(LPBYTE dest,BYTE *src, UINT widthPix, UINT height)
{
	if (src==NULL)
		return FALSE;

	UINT col, row,len=widthPix*8;
   LPBYTE dp=dest;
   dp+=len*(height-1)/2;
