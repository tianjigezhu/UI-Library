...
int ringDateTime::GetYear()
{
	GetTime();
	return (int)m_time.wYear;	
}

int ringDateTime::GetMonth()
{
	GetTime();
	return (int)m_time.wMonth;
}

int ringDateTime::GetDay()
{
	GetTime();
	return (int)m_time.wDay;
}

int ringDateTime::GetDayOfWeek()
{
	GetTime();
	return (int)m_time.wDayOfWeek;
}

int ringDateTime::GetHour()
{
	GetTime();
	return (int)m_time.wHour;
}

int ringDateTime::GetMinute()
{
	GetTime();
	return (int)m_time.wMinute;
}

int ringDateTime::GetSecond()
{
	GetTime();
...
int ringDateTime::ms()
{
	...
	{
		//不与系统时间绑定，则不能改变m_time的值
		GetLocalTime(&m_tmptime);
		return (int)m_tmptime.wMilliseconds;
	}   
}
...
LPCTSTR ringDateTime::formatStr(LPSTR s,int nval,int length)
{
	if(nval > 9 || length != 2)
		wsprintf(s,"%d\0",nval);
	else
		wsprintf(s,"0%d\0",nval);
	return s;
}

BOOL ringDateTime::SetDate(WORD month,WORD day,WORD year)
{
	GetTime();

	//100年后还会有人用这个程序吗？哈哈！
	//BTW：这个范围为系统调整年份时限定的范围。
...
BOOL ringDateTime::SetTime(WORD hour,WORD minute,WORD second)
{
	GetTime();
   if(hour < 24)
   	m_lptime->wHour = hour;
...
ringDateTime& ringDateTime::operator=(const ringDateTime& rdt)
{
	if(this != &rdt)
	{
...
ringDateTime& ringDateTime::operator=(const SYSTEMTIME* lptime)
{
	if(lptime != NULL)
	{
		m_bReCount = FALSE;
		memcpy(&m_time,lptime,sizeof(SYSTEMTIME));
		memcpy(&m_tmptime,lptime,sizeof(SYSTEMTIME));
	}
	return *this;
}

ringDateTime& ringDateTime::operator =(const FILETIME* lptime)
{
	if(lptime != NULL)
	{
		m_bReCount = FALSE;
		FileTimeToSystemTime(lptime,&m_time);		
	}
	return *this;
}

SYSTEMTIME* ringDateTime::IncMonth(int nStep,SYSTEMTIME* lptime/*=NULL*/)
{
	m_bReCount = FALSE;
...
//
**********************************************************/
/*LPCTSTR ringDateTime::FormatString(LPCTSTR szFormat)
{
	if(szFormat)
	{
		m_str = szFormat;
